...
void TaskWindow::on_button_joy_walk_clicked()
{
    qnode.joy_walk_cmd_msg_.walk_mode = 1;
    qnode.joy_walk_cmd_msg_.compensator_mode[0] = ui.checkBox_hip_compensator->isChecked();
    qnode.joy_walk_cmd_msg_.compensator_mode[1] = ui.checkBox_ext_encoder_cmp->isChecked();

    qnode.joy_walk_cmd_msg_.heel_toe = (ui.comboBox_heel_toe->currentText().toStdString() == "Yes");

    if(ui.comboBox_method->currentText().toStdString() == "Inverse Kinematics")
    {
      qnode.joy_walk_cmd_msg_.ik_mode = dyros_jet_msgs::WalkingCommand::IK;
    }
    else if (ui.comboBox_method->currentText().toStdString() == "Jacobian")
    {
      qnode.joy_walk_cmd_msg_.ik_mode = dyros_jet_msgs::WalkingCommand::JACOBIAN;
    }

    qnode.send_joy_walk_ctrl();
}
