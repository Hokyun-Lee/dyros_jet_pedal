...
ControlBase::ControlBase(ros::NodeHandle &nh, double Hz) :
  ...
  walking_controller_(model_, q_, q_dot_, torque_, Hz, control_time_),
  ...
  joint_control_as_(nh, "/dyros_jet/joint_control", false) // boost::bind(&ControlBase::jointControlActionCallback, this, _1), false
{
  ...
  walking_joy_ui_command_sub_ = nh.subscribe("/dyros_jet/walking_joy_command",3,&ControlBase::joyUiCommandCallback, this);
  joy_command_sub_ = nh.subscribe("/dyros_jet/joystick_walking_command", 3, &ControlBase::joyCommandCallback, this);
  parameterInitialize();
  model_.test();
}
...
void ControlBase::update()
{
  ...
  q_vjoint_dot.segment<DyrosJetModel::MODEL_DOF>(6) = q_dot_.head<DyrosJetModel::MODEL_DOF>();

  model_.updateKinematics(q_vjoint,q_vjoint_dot);  // Update end effector positions and Jacobians
  model_.updateFullJoint(q_vjoint);
  stateChangeEvent();
}
...
void ControlBase::stateChangeEvent()
{
  if(checkStateChanged())
  {
    ...
    else if(current_state_ == "Motion1")
    {
      std::cout << "Sss" << std::endl;
      q_target_state_ = q_init_state_;
      q_target_state_(12) = -90*DEG2RAD;

      joint_controller_.setTarget(12, q_target_state_(12), 5);
      joint_controller_.setEnable(12, true);
    //   desired_q_(i) = DyrosMath::cubic(current_time_, start_time_[i], end_time_[i], start_q_(i), target_q_(i), 0, 0);
    }
    else if(current_state_ == "Motion2")
    {
      std::cout << "Sss" << std::endl;
      q_target_state_ = q_init_state_;
      q_target_state_(12) = 0*DEG2RAD;
...
void ControlBase::walkingCommandCallback(const dyros_jet_msgs::WalkingCommandConstPtr& msg)
{
  ...
  if(msg->walk_mode == dyros_jet_msgs::WalkingCommand::STATIC_WALKING)
  {
    ...
    walking_controller_.setJoystickWalking(msg->walk_mode, msg->compensator_mode[0], msg->compensator_mode[1], msg->ik_mode, msg->heel_toe, msg->first_foot_step,
        msg->x, msg->y, msg->z, msg->height, msg->theta, msg-> step_length_x, msg-> step_length_y);// eh
...
void ControlBase::joyUiCommandCallback(const dyros_jet_msgs::WalkingCommandConstPtr& msg)
{
//  for (int i =0; i<2; i++)
//  {
//    compensate_ui_v_[i]=msg->compensator_mode[i];
//  }
//  ik_ui_mode_ = msg->ik_mode;
  std::cout<<"=== Joystick Walking READY ==="<<std::endl;
     vector<bool> compensate_v;
     compensate_v.reserve(2);
     ...
     {
       //walking_controller_.setEnable(false);
       //std::cout<<"compensator mode : "<<msg->compensator_mode[0]<<", "<<msg->compensator_mode[1]<<std::endl;
       //std::cout<<"step length : "<<msg->step_length_x<<std::endl;
       walking_controller_.setJoystickWalking(msg->walk_mode, msg->compensator_mode[0], msg->compensator_mode[1], msg->ik_mode, msg->heel_toe, msg->first_foot_step,
...
{
    walking_controller_.getJoystick(msg->x, msg->y, msg->theta);
}
