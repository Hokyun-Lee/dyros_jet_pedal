...
void mujoco_interface::sensorStateCallback(const mujoco_ros_msgs::SensorStateConstPtr &msg)
{
  ...
  Eigen::Vector3d imu_acc, imu_gyro;
    for(int i=0;i<msg->sensor.size();i++){
        ...
        if(msg->sensor[i].name=="Acc_Pelvis_IMU"){
            for(int j=0;j<3;j++){
                imu_acc(j) = msg->sensor[i].data[j];
            }
        }
        if(msg->sensor[i].name=="Gyro_Pelvis_IMU"){
            for(int j=0;j<3;j++){
                imu_gyro(j) = msg->sensor[i].data[j];
            }
        }
   ...
   accelometer_ = DyrosMath::lowPassFilter<3>(imu_acc,accelometer_, 1.0 / 200, 0.05);
   gyro_ = DyrosMath::lowPassFilter<3>(imu_gyro, gyro_, 1.0 / 200, 0.05 );
}
